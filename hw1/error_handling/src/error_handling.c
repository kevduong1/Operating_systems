...
{
	if(num_records == 0 || *records != NULL) return -1;
	if((int)num_records < 0) return -2;
...
int read_records(const char *input_filename, Record_t *records, const size_t num_records) 
{
	...
	if(sizeof(Record_t) * (records - &records[0] + 1) < num_records) return -3;
